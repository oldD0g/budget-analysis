...

{
	...
	global $dbuser, $dbpassword, $database, $transactionTable;
		...
		// On review I couldn't work out what this "like" style query was trying to do
		// so I replaced it with one that made more sense.
        ...
        if ($result = mysql_query($query,$myDB)) {
                while ($row = mysql_fetch_array($result,MYSQL_NUM)) {
						...
						sort($enum);
						$id = substr($name,-4);
                        echo "<SELECT name=$name onchange=\"settickon('$id')\">"; 
                        foreach ($enum as $option) {
                                echo "\t" . "<OPTION ";
                                if ($option == $initialvalue) {
                                	print "SELECTED ";
                                }
                                print 'value="'. $option.'">'. $option . "\n";
                        }
                        if ($initialvalue =="") {
                        	print "<OPTION SELECTED value=\"Not set\">Not set\n";
                        }
	...
	global $dbuser, $dbpassword, $database, $transactionTable;
        ...
        if ($result = mysql_query($query,$myDB)) {
                ...
                if ($result = mysql_query($query,$myDB)) {
                	...
                	print "Unable to update category for transaction ID $transid<br>\n";
                }
        } else {
                print "Could not run query against database\n<br>\n";
        }
