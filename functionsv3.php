...
if (! ($myDB = mysqli_connect("localhost",$dbuser,$dbpassword, $database))) {
...

{
	...
	global $dbuser, $dbpassword, $database, $transactionTable;
		...
		$result = mysqli_query($myDB,"SELECT COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS
...

{
    ...
    $myDB = mysqli_connect("localhost",$username,$password,"$database");
...

{
   ...
   if ($myDB->connect_error) {
        ...
        if ($result = mysqli_query($myDB,$query)) {
                ...
                if ($result = mysqli_query($query,$myDB)) {
...

{
   ...
   if ($myDB->connect_error) {
        ...
        if ($result = mysqli_query($myDB,$query)) {
        		...
        		if ( !$result = mysqli_query($query,$myDB)) { die("Couldn't show columns to get category names!"); }
        		
                if (! $row = mysqli_fetch_array($result,1)) { die ("Didn't find any rows in query result!"); }
                        /* The "Type" entry contains the list of options since that
                ...
                foreach ($affectedids as $ID) {
                  ...
                  if (! $result = mysqli_query($query,$myDB)) { echo("Failed to update category for ID $ID<br>\n"); }
                }
        } else {
                print "Could not run query against database\n<br>\n";
        }
...

{
    ...
    if ($myDB->connect_error) {
        ...
        $query="show columns from " . $transactionTable . " like 'transcat'";
      	if ( !$result = mysqli_query($myDB,$query)) { die("Couldn't show columns to get category names!"); }
...

{
   ...
   if ($myDB->connect_error) {
       ...
       $query="show columns from " . $transactionTable . " like 'transcat'";
        if ($result = mysqli_query($myDB,$query)) {
         if (! $row = mysqli_fetch_array($result,1)) { die("Couldn't get transcat description, fetch_array failed"); }
...

{
   ...
   if ($myDB->connect_error) {
       ...
       $query="select min(transdate) as mindate,max(transdate) as maxdate from " . $transactionTable;
        if ($result = mysqli_query($myDB,$query)) {
         if (! $row = mysqli_fetch_array($result,1)) { die("Couldn't get results from query, fetch_array failed"); }
